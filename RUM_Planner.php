  ...
  <script>
    ...
    function refreshKpisCharts(){
      ...
      state.tasks.forEach(t=>{
        ...
        byTrade[trade] = (byTrade[trade]||0) + cost;
        byPhase[phase] = (byPhase[phase]||0) + cost;
        byStatus[status] = (byStatus[status]||0) + 1;
      });

      const kpiBACEl = document.getElementById('kpiBAC');
      const kpiEVEl = document.getElementById('kpiEV');
      const kpiACEl = document.getElementById('kpiAC');
      ...
      const cpiChip = document.getElementById('cpiChip');
      const spiChip = document.getElementById('spiChip');
      const svChip = document.getElementById('svChip');

      if (kpiBACEl) kpiBACEl.textContent = money(bacTotal);
      if (kpiEVEl) kpiEVEl.textContent = money(evTotal);
      if (kpiACEl) kpiACEl.textContent = money(acTotal);
      if (errorCostEl) {
        errorCostEl.textContent = `Error 01: ${missingCost} actividades sin BAC`;
        errorCostEl.classList.toggle('is-active', missingCost > 0);
      }
      if (errorWbsEl) {
        errorWbsEl.textContent = `Error 02: ${missingWbs} actividades sin WBS`;
        errorWbsEl.classList.toggle('is-active', missingWbs > 0);
      }
      if (phaseSummary) {
      ...
      if (cpiChip) cpiChip.textContent = `CPI: ${CPI? CPI.toFixed(2) : '—'}`;
      if (spiChip) spiChip.textContent = `SPI: ${SPI? SPI.toFixed(2) : '—'}`;
      if (svChip) svChip.textContent = `SV: ${SV? money(SV) : '—'}`;
