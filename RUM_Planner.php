...
<div class="rum-planner-page">
  ...
  <script>
    ...
    function refreshKpisCharts(){
      ...
      window.__phaseTotals = {};
      window.__tradeTotals = {};
      state.tasks.forEach(t=>{
        const phase = t.phase || 'Sin fase';
        const trade = t.trade || 'Sin trade';
        const cost = Number(t.cost||0);
        window.__phaseTotals[phase] = (window.__phaseTotals[phase]||0) + cost;
        window.__tradeTotals[trade] = (window.__tradeTotals[trade]||0) + cost;
      });
      const phaseSummary = document.getElementById('phaseSummary');
      const tradeSummary = document.getElementById('tradeSummary');
      if (phaseSummary) {
        phaseSummary.innerHTML = Object.entries(window.__phaseTotals).map(([k,v])=>(
          `<li><span>${escapeHtml(k)}</span><strong>${money(v)}</strong></li>`
        )).join('') || '<li><span>Sin datos</span><strong>—</strong></li>';
      }
      if (tradeSummary) {
        tradeSummary.innerHTML = Object.entries(window.__tradeTotals).map(([k,v])=>(
          `<li><span>${escapeHtml(k)}</span><strong>${money(v)}</strong></li>`
        )).join('') || '<li><span>Sin datos</span><strong>—</strong></li>';
      }
      ...
      window.__missingCost = state.tasks.filter(t=>Number(t.cost||0) <= 0).length;
      window.__missingWbs = state.tasks.filter(t=>!String(t.wbs||'').trim()).length;
      document.getElementById('errorCost').textContent = `Error 01: ${window.__missingCost} actividades sin BAC`;
      document.getElementById('errorWbs').textContent = `Error 02: ${window.__missingWbs} actividades sin WBS`;
